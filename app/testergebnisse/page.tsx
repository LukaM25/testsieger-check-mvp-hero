    ...
    <div className="bg-gray-50">
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-6 pb-16 pt-24">
          ...
          <h1 className="mt-2 text-4xl font-bold text-slate-900 md:text-5xl">{tr('Aktuelle Testergebnisse', 'Current test results')}</h1>
        ...
        <section className="mx-auto max-w-6xl px-6 py-10">
          {certificate ? (
            <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              <h2 className="text-xl font-semibold text-slate-900">{tr('Verifikation für Produkt-ID', 'Verification for product ID')} {productId}</h2>
              <p className="text-sm text-slate-600">{tr('Siegelnummer', 'Seal number')}: <span className="font-mono">{certificate.seal_number}</span></p>
            ...
            <div className="rounded-3xl border border-orange-200 bg-orange-50 p-6 text-sm text-orange-900">
              <p>{tr('Keine gültige Prüfung für Produkt-ID', 'No valid test for product ID')} {productId} {tr('gefunden. Bitte überprüfen Sie die Siegelnummer oder wenden Sie sich an unseren Support.', 'found. Please check the seal number or contact our support.')}</p>
            </div>
          )}
        </section>
      ) : null}
          ...
          <div className="grid grid-cols-1 divide-y divide-slate-200">
            {results.map((entry) => (
              <div key={entry.product.de} className="grid gap-6 px-6 py-6 md:grid-cols-[2fr,1fr,1fr] md:items-center">
                <div>
                  <h2 className="text-lg font-semibold text-slate-900">{tr(entry.product.de, entry.product.en)}</h2>
      ...
      <section className="border-t border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-6 py-16">
          <h2 className="text-2xl font-semibold text-slate-900">{tr('Siegelstufen', 'Seal levels')}</h2>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-3">
            {badges.map((badge) => (
              <div key={badge.title.de} className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
                ...
                <h3 className="mt-2 text-xl font-semibold text-slate-900">{tr(badge.title.de, badge.title.en)}</h3>
